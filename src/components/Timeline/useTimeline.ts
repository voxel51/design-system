...
export function useTimeline({
  ...
  // Ref so the setInterval callback always sees fresh loop bounds without recreating the interval.
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      setCurrentTime((prev) => {
        ...
        return next >= loopRef.current.loopEnd ? loopRef.current.loopStart : next;
      });
    }, 1000 / fps);
