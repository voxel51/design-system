...
import { Anchor, Variant } from "@/types";
...
describe("Toast", () => {
  ...
  it("should give link actions the solid on-color styling", () => {
    render(
      <Toast
        {...defaultProps}
        solid
        variant={Variant.Success}
        action={<a href="/datasets">View dataset</a>}
      />
    );

    const controls = screen.getByRole("link", { name: "View dataset" })
      .parentElement as HTMLElement;
    expect(controls.className).toContain("[&_:is(a,button)]:text-current");
  });

  it("should ignore solid on a variant without an action color", () => {
    render(<Toast {...defaultProps} solid variant={Variant.Secondary} />);

    expect(screen.getByTestId(testId).className).not.toContain("action-");
  });
  ...
  describe("duration", () => {
    ...
    it("should keep holding for focus after the pointer leaves", () => {
      const onClose = jest.fn();
      render(
        <Toast
          {...defaultProps}
    ...
    it("should keep holding while focus moves between its own controls", () => {
      const onClose = jest.fn();
      render(
        <Toast
          {...defaultProps}
    ...
    it("should resume once focus leaves it", () => {
      const onClose = jest.fn();
      render(
        <Toast
          {...defaultProps}
